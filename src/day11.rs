...
impl crate::aoc::AoCSolution for Day11 {
    ...
    fn convert(&self, input: &str) -> Self::ConvertedType {
        input.lines().map(|x| x.chars().map(|v| v.to_digit(10)).flatten().collect()).collect()
    }
    ...
    fn part1(&self, input: &Self::ConvertedType) -> Self::ReturnType {
        ...
        let mut flashes = 0;
        for _ in 0..100 {
            input.iter_mut().for_each(|v| v.iter_mut().for_each(|x| *x += 1));
            ...
            input.iter_mut().for_each(|v| v.iter_mut().for_each(|x| if *x > 9 { *x = 0 }));
            flashes += flashed_already.len() as u64;
        }
        flashes
    }
